...
export function AdminDashboard({ view }: AdminDashboardProps) {
    ...
    function AdminMonitoring({ reports, schools }: { reports: Report[], schools: SchoolData[] }) {
        ...
        const refinedComplianceData = KECAMATAN_LIST.map(kec => {
            ...
            // 2. Secondary: If percentages are equal and > 0, Smaller timestamp (earlier) is better
            if (a.percentage > 0 && b.percentage === a.percentage) {
                                ...
                                <tbody className="divide-y divide-slate-100 bg-white">
                                    {pendingReports.map(report => (
                                        <tr key={report.id} className="hover:bg-slate-50 transition-colors">
                                            ...
                                            <td className="px-6 py-4">
                                                <a href={report.link} target="_blank" rel="noreferrer" className="text-blue-600 hover:text-blue-800 hover:underline flex items-center gap-1 font-medium">
    ...
    function AdminSettings({ refresh }: { refresh?: () => void }) {
        ...
        const handleResetKorwil = async (id: string) => {
            if (!confirm("Reset password Korwil menjadi: korwil123 ?")) return;
            try {
                setLoadingKorwil(true);
                await firebaseService.resetUserPassword(id, 'korwil123');
                toast.success("Password direset menjadi: korwil123");
            } catch (e: any) {
                                            ...
                                            korwilAccounts.map((korwil) => (
                                                <tr key={korwil.id} className="hover:bg-slate-50/50">
                                                    ...
                                                    <td className="px-4 py-3 text-center">
                                                        <div className="flex justify-center gap-1">
                                                            <button onClick={() => handleResetKorwil(korwil.id)} className="p-1.5 text-amber-600 hover:bg-amber-50 rounded transition-colors" title="Reset Password ke korwil123"><Lock size={16} /></button>
